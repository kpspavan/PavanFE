...
const FeatureProjects = ({ type, title, summary,link, img, github }) => {
  return (
    <article
      className="w-full dark:bg-dark dark:border-light flex items-center justify-between rounded-3xl border border-solid border-dark
      relative
         bg-light
         p-12
    ...
    >
      ...
      <div className="w-1/2 flex flex-col items-start justify-between pl-6 " >
        ...
        <div className="mt-2 flex items-center  " >
          ...
          <Link href={link} 
          className="ml-4 dark:bg-light dark:text-dark rounded-lg bg-dark text-light p-2 px-6 text-lg font-semibold  "
                    target="_blank">
            Visit project
          </Link>
        </div>
...
const Projects = ({ type, title,link, img, github }) =>{
    return (
        <article
        className="w-full dark:bg-dark  flex items-center justify-center 
        rounded-2xl border dark:border-light border-solid 
        flex-col border-dark bg-light p-6 relative  "
      >
      <div className="absolute top-0 dark:bg-light -right-3 -z-10 w-[101%] h-[103%] rounded-[2rem] bg-dark rounded-br-3xl " />
